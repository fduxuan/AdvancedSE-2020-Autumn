...
class Model:
    ...
    def find_one(self, query=None, to_raise=False):
        """
        get a mongo object
    ...
    def insert_array_field_at_start(self, query, array_field, insert_eles):
        self.coll.update_one(query, {"$push": { "{f}".format(f=array_field): { "$each": insert_eles, "$position": 0 } } })

    def insert_inner_array_field_at_start(self, query, outer_array_field, inner_array_field, insert_eles):
        self.coll.update_one(query, {"$push": {"{o_f}.$.{i_f}".format(o_f=outer_array_field, i_f=inner_array_field): { "$each": insert_eles, "$position": 0 }}})